<#
    ...
    Exported Functions
    1. Get-TBAAllEventList
    2. Get-TBAAllTeamList
    3. Get-TBAEventCOPR
    4. Get-TBAEventOPR
    5. Get-TBAEventRanking
    6. Get-TBAEventTeamList
    7. Get-TBAJsonAPIKey
    8. Get-TBAJsonConfig
    9. Get-TBAJsonEventKey
    10. Get-TBAJsonFilePath
    11. Get-TBAJsonTeamKey
    12. Get-TBATeamEventMatchInfo
    13. Get-TBATeamEventOPR
    14. Get-TBATeamName
    15. New-TBAJsonConfigFile
    16. Test-TBAStatus
    17. Update-TBAJsonAPIKey
    18. Update-TBAJsonEventKey
    19. Update-TBAJsonTeamKey
    ...
    3. Get-TBAHeader
    4. Invoke-TBARetry
    5. Update-TBAJsonField
#>

######################
# Exported Functions #
######################
function Get-TBAAllEventList {
    <#
        .SYNOPSIS
    ...
    try {
        ...
        $FRCEvents = Get-TBAData @GetTBADataParams | ConvertFrom-Json
    }
    catch {
        Write-Error "Unable to pull TBA data"
    }
    ...
    foreach ($FRCEvent in $FRCEvents) {
        ...
        if ($null -ne $FRCEvent.week) {
            # TBA week is 0-based; display 1-based
            $Include = $true
            $WeekLabel = [int]$FRCEvent.week + 1
            $WeekSort = $WeekLabel
        }
        ...
        elseif ($FRCEvent.event_type_string -in @('Championship Division', 'Championship Finals')) {
            $Include = $true
            $WeekLabel = 8
            $WeekSort = 100
        }
        ...
        if ($Include) {
            $Output += [PSCustomObject]@{
                ...
                event_week        = $WeekLabel

                # Internal sort key; removed after sorting
                event_week_sort   = $WeekSort
            }
        }
    }

    $Output |
    Sort-Object event_week_sort, event_name |
    Select-Object event_name, event_key, event_type, event_type_string, event_week
}

function Get-TBAAllTeamList {
    <#
        .SYNOPSIS
...
function Get-TBAEventCOPR {
    <#
        ...
        .EXAMPLE
            Get-TBAEventCOPR -EventKey "2026ohcl"

            Retrieves COPR data from the 2026 Buckeye Regional and returns a list of teams with their
            component OPR values.

        .EXAMPLE
            "2026ohcl" | Get-TBAEventCOPR

            Uses pipeline input to retrieve COPR data for the 2026 Buckeye Regional.
...
function Get-TBAEventOPR {
    ...
    .EXAMPLE
        ...
        This assumes the event key is stored in the TBA.json file.

    .EXAMPLE
        Get-TBAEventOPR -EventKey "2026ohcl"

    .EXAMPLE
        "2026ohcl" | Get-TBAEventOPR
...
function Get-TBAJsonAPIKey {
    <#
        ...
        .EXAMPLE
            Get-TBAJsonAPIKey -JsonFilePath "C:\Configs\tba.json"
...
function Get-TBAJsonEventKey {
    <#
        ...
        .EXAMPLE
            Get-TBAJsonEventKey -JsonFilePath "C:\Configs\tba.json"
...
function Get-TBAJsonFilePath {
    ...
    try {
        $JsonConfigModule = Get-Module -ListAvailable TBAInfo -ErrorAction Stop | Select-Object -First 1
...
function Get-TBAJsonTeamKey {
    <#
        ...
        .EXAMPLE
            Get-TBAJSONTeamKey -JsonFilePath "C:\Configs\TBA.json"
...
function New-TBAConfigFile {
    ...
    .DESCRIPTION
    This function creates a JSON file named 'TBA.json' containing the TBA API key, event key, and team key.
    ...
    .EXAMPLE
        ...
        Prompts the user to enter API key, event key, and team key, then saves them to 'TBAjson'.

    .EXAMPLE
        New-TBAConfigFile -ApiKey "abc123" -EventKey "2025ohcl" -TeamKey "frc254"

        Saves the provided values directly to 'TBA.json' without prompting.
    ...
    $config | ConvertTo-Json -Depth 3 | Set-Content -Path "./TBA.json" -Encoding UTF8

    Write-Output "Configuration saved to TBA.json"
}
...
function Update-TBAJsonApiKey {
    <#
        ...
        .DESCRIPTION
            This is a wrapper around Update-TBAJsonField that simplifies updating
    ...
    process {
        try {
            Write-Verbose "Calling Update-TBAJsonField to update api_key"
            Update-TBAJsonField -ApiKey $ApiKey -ErrorAction Stop
...
function Update-TBAJsonEventKey {
    ...
    process {
        try {
            Write-Verbose "Calling Update-TBAJsonField to update event_key"
            Update-TBAJsonField -EventKey $EventKey -ErrorAction Stop
...
function Update-TBAJsonTeamKey {
    ...
    try {
        ...
        Write-Verbose "Calling Update-TBAJsonField to update team_key"
        Update-TBAJsonField -TeamKey $TeamKey -ErrorAction Stop
...
function Test-TBAStatus {
    <#
        ...
        .DESCRIPTION
            ...
            using the base URL and headers provided by the `Get-TBAHeader` function.
        ...
        .OUTPUTS
            System.Boolean
            $true:  If the TBA API is reachable and responding with status code 200
            $false: If not

        .EXAMPLE
            Test-TBAStatus
            True (Assuming TBA is available)
    ...
    try {
        ...
        $TBARequest = Invoke-WebRequest @InvokeWebRequestParams
...
# Private Functions  #
######################
function Get-TBABaseURL {
    [CmdletBinding()]
    [OutputType([string])]
    param()

    $BaseURL = "https://www.thebluealliance.com/api/v3/"
    Write-Verbose "TBA Base URL:"
...
function Update-TBAJsonField {
    <#
        .SYNOPSIS
            Updates one or more fields (team_key, event_key, api_key) in a TBA JSON file.

        .DESCRIPTION
            Reads a JSON config file for The Blue Alliance (TBA), updates specified fields,
            and writes the updated JSON back to disk.

        .PARAMETER TeamKey
            Team identifier in the format "frcXXXX" or "XXXX" (number only).
            If a number is provided, it will be normalized to "frcXXXX".

        .PARAMETER EventKey
            Event identifier (e.g., "2025ohcl").

        .PARAMETER ApiKey
            TBA API key string.

        .PARAMETER JsonFilePath
            Path to the JSON file to update. Defaults to Get-TBAJsonFilePath.

        .PARAMETER PassThru
            Returns the updated JSON object to the pipeline.

        .EXAMPLE
            Update-TBAJsonField -TeamKey 1234 -EventKey 2025ohcl -Verbose
    ...
    begin {
        # Build a map of JSON properties to update (only those actually provided)
        $updates = [ordered]@{}

        if ($PSBoundParameters.ContainsKey('TeamKey')) {
            # Normalize: "1234" -> "frc1234"
            $normalizedTeamKey = if ($TeamKey -match '^frc') { $TeamKey } else { "frc$TeamKey" }
            $updates['team_key'] = $normalizedTeamKey
        }
        if ($PSBoundParameters.ContainsKey('EventKey')) { $updates['event_key'] = $EventKey }
        if ($PSBoundParameters.ContainsKey('ApiKey'))  { $updates['api_key']  = $ApiKey  }

        if ($updates.Count -eq 0) {
            throw "No fields specified to update. Provide -TeamKey, -EventKey, and/or -ApiKey."
        }

        # Resolve + validate target file early
        try {
            $resolvedPath = (Resolve-Path -LiteralPath $JsonFilePath -ErrorAction Stop).Path
        }
        catch {
            throw "File not found or invalid path: $JsonFilePath"
        }
    ...
    process {
        Write-Verbose "Loading JSON from '$resolvedPath'"
        ...
        foreach ($k in $updates.Keys) {
            Write-Verbose "Updating '$k' to '$($updates[$k])'"
            $json.$k = $updates[$k]
        }

        $action = "Update TBA JSON fields: $($updates.Keys -join ', ')"

        if ($PSCmdlet.ShouldProcess($resolvedPath, $action)) {
            try {
                # PowerShell 7+: be explicit about encoding; utf8NoBOM is the usual JSON preference
                $json |
                    ConvertTo-Json -Depth 20 |
                    Set-Content -LiteralPath $resolvedPath -Encoding utf8NoBOM -ErrorAction Stop
            }
            catch {
                $PSCmdlet.ThrowTerminatingError($_)
            }
